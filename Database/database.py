...
class Database:
    ...
    async def insert_server(self, game_type, address, port, channel_id, country, lang):
    ...
    async def update_server(self, server_id, game_type, address, port, channel_id, country, lang):
        conn, cursor = self.cursor()
    ...
    async def update_server_message_id(self, server_id, message_id):
        conn, cursor = self.cursor()
        cursor.execute(statements.PreparedStatements.ADD_SERVER_MESSAGE_ID.__str__(), (message_id, server_id))
        conn.commit()
        self.close(conn, cursor)

    async def update_server_name(self, server_id, name):
        conn, cursor = self.cursor()
    ...
    async def get_server_by_message_id(self, message_id):
        conn, cursor = self.cursor()
    ...
    async def get_server_by_address_port(self, address, port):
        conn, cursor = self.cursor()
